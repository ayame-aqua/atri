...
| 最近修订 | v0.11 |
| 变更记录 | [变更/](./变更/) |
| 依赖 | **执行顺序：** P7 已验收。**逻辑硬依赖：** P2-A（同一记忆）。P4 不是 QQ 文字的逻辑前提，但本仓库不并行开工 QQ 登录，争论时只看总纲 §3.1 |
| 下一阶段 | [P6-微信通道.md](./P6-微信通道.md) |
| 总纲 | [开发文档.md](./开发文档.md) |
...
本机桌宠的「连续聊 + 主动找你 + 看屏幕」**已经在 P7 做完**，本阶段不要把看屏接到群里。

进入本阶段才选怎么连 QQ。**P5 之前**不得出现登录器。P5 之前允许的预研：读官方文档、写选型笔记、用群导出文本跑 P2-B 导入脚本。
...
| B. 专用个人号 + 网关 | 普通好友 | 更接近真表情包收发；P5 再定网关 |
...
- 想等：`qq_wait`（P5 可先用超时唤醒代替）

P5 第一版允许「规则唤醒 + 模型 silent」。观望/活跃状态机也在本阶段做完第一版，**不要留到后面推翻协议**。`silent` 只来自 `[[atri]]` 尾块（或工具未调用发送），解析失败时按总纲降级为 `silent=false` 前，QQ 策略仍可选择不 `send`（通道限频与白名单优先于模型胡言）。
...
- P6：个人微信文字通道；必须先做合规可行性确认；不要上企业微信 / 公众号
- 桌宠与 CV 已在 P7 完成；不要把 `see_screen` 接到 QQ
