...
| --- | --- |
| 阶段 | P5 |
| 依赖 | P1–P4 验收通过（P4 不阻塞 QQ 文字，但建议先做完） |
| 下一阶段 | [P6-企业微信或公众号.md](./P6-企业微信或公众号.md) |
| 总纲 | [开发文档.md](./开发文档.md) |
| 原则 | [双模式说明.md](./双模式说明.md) |
| 本阶段目标 | QQ 里她是会自己决定说不说的群友：能发/收/生成表情包；不是桌宠那种不停找你聊 |
...
本机桌宠的「连续聊 + 主动找你 + 看屏幕」**不是本阶段**，见 P7。

进入本阶段才选怎么连 QQ。P1–P4 不得出现登录器。
...
P5 第一版允许「规则唤醒 + 模型 silent」。P7 再加观望/活跃状态机调参，**协议不要推翻**。
...
- P6：微信先文字通道；不要复制表情 MCP 除非很容易
- P7：桌宠陪伴与 CV；QQ 只把 `policy/qq.py` 调得更像人（状态机），**不要改回条条必回**
