...
- [x] `channels/qq.py` 空壳；`start` 无登录；`send` 明确 NotImplemented
- [x] `policy/qq.py` 占位（`DEFAULT_SILENT_OK`）
- [x] `OutboundMessage.silent`、`sticker_ids` 字段已有
- [x] `channels.qq.enabled: false`
- [x] 总纲：P5 未验收不得写 QQ 登录

### 未勾

- [ ] 选定官方机器人 vs 专用号网关
- [ ] 收发文字；白名单 fail-closed
- [ ] 群默认不回；@ / 点名才回；silent 一条都不发
- [ ] 表情包库 + 收集群 U 表情 + 管理页
- [ ] MCP 生成表情并发到白名单
- [ ] 限频；学习队列与发言分开
- [ ] 不接截屏、不把桌宠主动逻辑同步到群
...
P6 第一版允许「规则唤醒 + 模型 silent」。观望/活跃状态机也在本阶段做完第一版，**不要留到后面推翻协议**。`silent` 只来自 `[[natsume]]` 尾块（或工具未调用发送），解析失败时按总纲降级为 `silent=false` 前，QQ 策略仍可选择不 `send`（通道限频与白名单优先于模型胡言）。
