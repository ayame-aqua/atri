...
| 变更记录 | [变更/](../../变更/) |
| 借鉴 | [借鉴.md](../借鉴.md) | |
| 依赖 | P3-A 验收通过（因而已有 P1 与 P2-A；不要等麦克风） |
...
本机真聊台词（人设 / 记忆 / 五态 / 口型 / 跨重启）见 [P4 跑通后对话集](../../测试/P4跑通后对话集.md)。
...
- **驱法**：AIRI 的 `pixi-live2d-display`。口型用 P3-A 的 wav 做 RMS，不要只靠 Artemis 那种 `/api/speak start|end` 张嘴。
- **情绪**：只认 `[[natsume]]` 的五态，自己做 `emotionMap`。不要抄 Artemis 夏目皮的官方 motion 名（摸头/摸胸那张表）。
- **桥**：P4 先做进聊天页。P5 桌宠窗若要独立进程，再加薄 HTTP/WS（学 Artemis 桥很薄，不学它的整仓 `live2d/`）。
- **降级**：无模型、加载失败，聊天和语音仍可用（Artemis / 本方案一致）。
- **资源**：有权的 natsume 皮自备。Artemis 打包的官方模型和 Cubism Core **不准进 git**。
